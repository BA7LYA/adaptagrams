...
/* lifted wholely from wikipedia.  Well, apart from the bug in the wikipedia
 * version. */

static void matrix_times_vector(
    const std::valarray<double>& matrix, /* m * n */
    const std::valarray<double>& vec,    /* n */
...
#if defined(_MSC_VER)
    // magmy: The following lines show how operator[] is defined for
    // std::valarray under MSVC _Ty std::valarray<_Ty>::operator[](size_t _Off)
    // const; _Ty &valarray<_Ty>::operator[](size_t _Off); As a consequence, it
    // is not possible to take the address of a constant std::valarray[n]. This
    // looks like a bug in the Microsoft's <valarray> file. Below is a
    // workaround
    const double* mp = &const_cast<std::valarray<double>&>(matrix)[0];
#else
...
)
{
    ...
    std::valarray<double> Ap(n), p(n), r(n);
