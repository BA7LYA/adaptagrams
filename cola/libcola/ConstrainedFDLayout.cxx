...
#include "libcola/ConstrainedFDLayout.hxx"
...
ConstrainedFDLayout::ConstrainedFDLayout(
    ...
    : n(rs.size())
    , X(std::valarray<double>(n))
    , Y(std::valarray<double>(n))
    , done(doneTest)
    ...
    , m_nonoverlap_exemptions(new NonOverlapConstraintExemptions())
{
    ...
    unsigned i = 0;
    for (vpsc::Rectangles::const_iterator ri = rs.begin(); ri != rs.end();
         ++ri, ++i)
    {
        X[i] = (*ri)->getCentreX();
        Y[i] = (*ri)->getCentreY();
...
void ConstrainedFDLayout::computeNeighbours(std::vector<Edge> es)
{
    for (unsigned i = 0; i < n; ++i)
    {
        neighbours.push_back(std::vector<unsigned>(n));
    }
...
)
{
    ...
    for (std::vector<Edge>::const_iterator e = es.begin(); e != es.end(); ++e)
    {
...
)
{
    for (std::vector<Cluster*>::iterator curr = cluster->clusters.begin();
         curr != cluster->clusters.end();
         ++curr)
    {
        // For each of the child clusters, recursively call this function.
        recGenerateClusterVariablesAndConstraints(
            vars,
            priority,
            noc,
            *curr,
    ...
    if ((noc == nullptr) && (dynamic_cast<RootCluster*>(cluster) == nullptr))
    {
        ...
        // Left:
        variable = new vpsc::Variable(
            vars[XDIM].size(),
            cluster->bounds.getMinX(),
            freeWeight
        );
        ...
        cola::ClusterContainmentConstraints* ccc
            = new cola::ClusterContainmentConstraints(
                cluster,
                priority,
                boundingBoxes
            );
    ...
    if (noc)
    {
        ...
        // level.
        // printf("Cluster #%d non-overlap constraints - nodes %d clusters
        // %d\n",
             ...
             ++curr)
        {
            ...
            if (cluster->m_overlap_replacement_map.count(id) > 0)
            {
                ...
                expandedClusterSet.insert(cluster->m_overlap_replacement_map[id]
                );
            }
        ...
        for (std::set<unsigned>::iterator curr
             ...
             curr != cluster->m_nodes_replaced_with_clusters.end();
             ++curr)
        {
            unsigned id = *curr;
            noc->addShape(
...
)
{
    if (clusterHierarchy && !clusterHierarchy->flat())
    {
        ...
        for (unsigned int i = 0; i < nodesInClusterCounts.size(); ++i)
        {
            ...
            if (!clusterHierarchy->allowsMultipleParents() && count > 1)
            {
                fprintf(
                    stderr,
...
void ConstrainedFDLayout::makeFeasible(double xBorder, double yBorder)
{
    ...
    for (unsigned int dim = 0; dim < 2; ++dim)
    {
        ...
        {
            double pos = (dim == 0) ? boundingBoxes[i]->getCentreX()
    ...
    idleConstraints.insert(
        idleConstraints.end(),
        extraConstraints.begin(),
        extraConstraints.end()
    );

    std::sort(
        idleConstraints.begin(),
        idleConstraints.end(),
    ...
    while (!idleConstraints.empty())
    {
...
#ifdef MAKEFEASIBLE_DEBUG
        {
            ...
            filename << "out/file-" << std::setfill('0') << std::setw(5)
                     << iteration << ".pdf";
...
void ConstrainedFDLayout::moveTo(const vpsc::Dim dim, Position& target)
{
    ...
    if (preIteration)
    {
        for (vector<Lock>::iterator l = preIteration->locks.begin();
             l != preIteration->locks.end();
             l++)
        {
            des.push_back(make_pair(l->getID(), l->pos(dim)));
            FILE_LOG(logDEBUG1)
                << "desi: v[" << l->getID() << "]=(" << l->pos(vpsc::HORIZONTAL)
                << "," << l->pos(vpsc::VERTICAL) << ")";
        }
    }
 ...
 */
double ConstrainedFDLayout::applyForcesAndConstraints(
    const vpsc::Dim dim,
    const double    oldStress
)
{
    FILE_LOG(logDEBUG
    ) << "ConstrainedFDLayout::applyForcesAndConstraints(): dim="
      << dim;
    valarray<double>      g(n);
    valarray<double>&     coords = (dim == vpsc::HORIZONTAL) ? X : Y;
    DesiredPositionsInDim des;
    ...
    if (topologyAddon->useTopologySolver())
    {
        stress = topologyAddon->applyForcesAndConstraints(
            this,
            dim,
            g,
            vs,
    ...
    FILE_LOG(logDEBUG
    ) << "ConstrainedFDLayout::applyForcesAndConstraints... done, stress="
      << stress;
    if (clusterHierarchy)
    {
...
)
{
    ...
    while (fabs(stepsize) > 0.00000000001)
    {
        ...
        // oldStress,stress,stepsize); if(oldStress>=stress) {
        return stress;
        //}
        coords    = oldCoords;
        stepsize *= 0.5;
...
)
{
    ...
    for (unsigned u = 0; u < n; u++)
    {
        ...
        for (unsigned v = 0; v < n; v++)
        {
            ...
            if (m_useNeighbourStress && neighbours[u][v] != 1)
            {
                continue;
            }

            // The following loop randomly displaces nodes that are at identical
            // positions
            double   rx = X[u] - X[v], ry = Y[u] - Y[v];
            double   sd2          = rx * rx + ry * ry;
            unsigned maxDisplaces = n;  // avoid infinite loop in the case of
                                        // numerical issues, such as huge values
            ...
            if (l > d && p > 1)
            {
                continue;  // attractive forces not required
            }
            double d2 = d * d;
    ...
    if (desiredPositions)
    {
        for (DesiredPositions::const_iterator p = desiredPositions->begin();
             p != desiredPositions->end();
             ++p)
        {
            unsigned i  = p->id;
            double   d  = (dim == vpsc::HORIZONTAL) ? p->x - X[i] : p->y - Y[i];
            d          *= p->weight;
            g[i]       -= d;
            H(i, i)    += p->weight;
        }
...
double ConstrainedFDLayout::computeStress() const
{
    ...
    for (unsigned u = 0; (u + 1) < n; u++)
    {
        for (unsigned v = u + 1; v < n; v++)
        {
            ...
            if (l > d && p > 1)
            {
                continue;  // no attractive forces required
            }
            double d2  = d * d;
            double rl  = d - l;
            double s   = rl * rl / d2;
    ...
    if (preIteration)
    {
        if ((*preIteration)())
        {
            for (vector<Lock>::iterator l = preIteration->locks.begin();
                 ...
                 l++)
            {
                double dx  = l->pos(vpsc::HORIZONTAL) - X[l->getID()],
                       dy  = l->pos(vpsc::VERTICAL) - Y[l->getID()];
                double s   = 1'0000 * (dx * dx + dy * dy);
                stress    += s;
...
void ConstrainedFDLayout::outputInstanceToSVG(std::string instanceName)
{
    ...
    for (size_t i = 0; i < boundingBoxes.size(); ++i)
    {
        fprintf(
            ...
            boundingBoxes[i]->getMinX(),
            boundingBoxes[i]->getMaxX(),
            boundingBoxes[i]->getMinY(),
            boundingBoxes[i]->getMaxY()
        );
    ...
    for (cola::CompoundConstraints::iterator c = ccs.begin(); c != ccs.end();
         ++c)
    {
