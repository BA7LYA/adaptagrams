...
#include "libvpsc/PairingHeap.hxx"
...
struct CompareNodes
{
    bool operator()(Node<T>* const& u, Node<T>* const& v) const
    {
        if (u == v)
        {
            return false;  // with g++ 4.1.2 unless I have this explicit check
                           // it returns true for this case when using -O3
                           // optimization CRAZY!
        }
        if (u->d < v->d)
        {
...
void neighbours(
    ...
    const std::valarray<T>&  eweights = std::valarray<T>()
);
...
)
{
    ...
    for (unsigned i = 0; i < n; i++)
    {
        for (unsigned j = 0; j < n; j++)
        {
            ...
            {
                D[i][j] = std::numeric_limits<T>::max();
            }
        }
    }
...
template<typename T>
void dijkstra(const unsigned s, std::vector<Node<T>>& vs, T* d)
{
    const unsigned n = vs.size();
    COLA_ASSERT(s < n);
    for (unsigned i = 0; i < n; i++)
    ...
    while (!Q.isEmpty())
    {
        ...
        for (unsigned i = 0; i < u->neighbours.size(); i++)
        {
            ...
            T        w = u->nweights[i];
            if (u->d != std::numeric_limits<T>::max() && v->d > u->d + w)
            {
                v->p = u;
                v->d = u->d + w;
