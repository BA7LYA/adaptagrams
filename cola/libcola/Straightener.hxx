...
public:
    ...
    // if the edge route intersects with any of the rectangles in rects then
    // reroute those parts of the route around the rectangle boundaries
    void rerouteAround(const std::vector<vpsc::Rectangle*>& rects)
    {
        unsigned rid = 0;
        for (std::vector<vpsc::Rectangle*>::const_iterator r = rects.begin();
             r != rects.end();
             r++, rid++)
        {
    ...
    void createRouteFromPath(const std::vector<Node*>& nodes);
    ...
    void xpos(double y, std::vector<double>& xs) const
    {
        ...
        {
            double ax = route->xs[i - 1], bx = route->xs[i],
                   ay = route->ys[i - 1], by = route->ys[i];
            double r = (y - ay) / (by - ay);
...
public:
    ...
    double                computeStress(const std::valarray<double>& coords);
    double                computeStress2(const std::valarray<double>& coords);
...
public:
    ...
    bool active;  // node is active if it is not dummy or is dummy and involved
                  // in a violated constraint
    bool open;    // a node is opened (if scan is true) when the scanline first
                // reaches its boundary and closed when the scanline leaves it.

    Node(unsigned id, const vpsc::Rectangle* r)
        : ScanObject(id)
        , cluster(nullptr)
        , edge(nullptr)
        , dummy(false)
    ...
    double euclidean_distance(const Node* v) const
    {
...
private:
    friend void sortNeighbours(
        const vpsc::Dim           dim,
        Node*                     v,
        Node*                     l,
        Node*                     r,
        const double              conjpos,
        const std::vector<Edge*>& openEdges,
        std::vector<Node*>&       L,
        std::vector<Node*>&       nodes
    );
...
{
    LinearConstraint(const Node& u, const Node& v, const Node& b, double w)
        : u(u.id)
        , v(v.id)
        , b(b.id)
        , w(w)
    {
        ...
        double uvx = v.pos[0] - u.pos[0], uvy = v.pos[1] - u.pos[1],
               ubx = b.pos[0] - u.pos[0], uby = b.pos[1] - u.pos[1],
...
double pathLength(const Edge* e, const std::vector<Node*>& nodes);
