...
#include "libcola/GradientProjection.hxx"
...
GradientProjection::GradientProjection(
    ...
    : k(k)
    , denseSize(static_cast<unsigned>(
          (floor(sqrt(static_cast<double>(denseQ->size()))))
      ))
    , denseQ(denseQ)
    , rs(rs)
    , ccs(ccs)
    ...
    , scaling(scaling)
{
    ...
    if (scaling)
    {
        ...
        for (unsigned i = 0; i < denseSize; i++)
        {
            ...
            {
                scaledDenseQ[i * denseSize + j] = (*denseQ)[i * denseSize + j]
                                                * vars[i]->scale
                                                * vars[j]->scale;
            }
        }
    ...
    if (ccs)
    {
             ...
             ++c)
        {
            ...
            OrthogonalEdgeConstraint* e
                = dynamic_cast<OrthogonalEdgeConstraint*>(*c);
            if (e)
            {
...
) const
{
    ...
    const double numerator   = dotProd(g, d);
...
unsigned GradientProjection::solve(
    const valarray<double>& linearCoefficients,
    valarray<double>&       x
)
{
...
#endif
    // it may be that we have to consider dummy vars, which the caller didn't
    // know about.  Thus vars.size() may not equal x.size()
    ...
    for (unsigned i = 0; i < x.size(); i++)
    {
        ...
        {
            vars[i]->desiredPosition = result[i];
        }
    }
    runSolver(result);
...
IncSolver* GradientProjection::setupVPSC()
{
    if (nonOverlapConstraints != None)
    {
        ...
        {
            for (vector<OrthogonalEdgeConstraint*>::iterator i
                 = orthogonalEdges.begin();
                 i != orthogonalEdges.end();
                 i++)
            {
            ...
            if (k == HORIZONTAL)
            {
                // Make rectangles a little bit wider when processing
                // horizontally so that any overlap resolved horizontally is
                // strictly non-overlapping when processing vertically
                Rectangle::setXBorder(0.0001);
                // use rs->size() rather than n because some of the variables
                // may be dummy vars with no corresponding rectangle
                generateXConstraints(
                    *rs,
                    vars,
                    lcs,
                    nonOverlapConstraints == Both ? true : false
                );
    ...
    case Off: break;
#ifdef MOSEK_AVAILABLE
    case Inner: menv = mosek_init_sep_ls(vars.size(), cs); break;
    case Outer:
        unsigned n   = vars.size();
...
void GradientProjection::destroyVPSC(IncSolver* vpsc)
{
    ...
    if (unsatisfiableConstraints)
    {
        ...
        for (Constraints::iterator i = cs.begin(); i != cs.end(); i++)
        {
            ...
            if (c->unsatisfiable)
            {
                UnsatisfiableConstraintInfo* ci
...
void GradientProjection::straighten(
    ...
    const vector<SeparationConstraint*>& cs,
    const vector<straightener::Node*>&   snodes
)
{
    ...
    for (vector<SeparationConstraint*>::const_iterator i = cs.begin();
         i != cs.end();
         i++)
    {
