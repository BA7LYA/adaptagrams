...
void Cluster::computeBoundingRect(const vpsc::Rectangles& rs)
{
    ...
    for (set<unsigned>::const_iterator i = nodes.begin(); i != nodes.end(); ++i)
    {
...
) const
{
    ...
    cola::SeparationConstraint* sc = new cola::SeparationConstraint(
        vpsc::XDIM,
        clusterVarId,
        m_rectangle_index,
        halfWidth,
    ...
    sc = new cola::SeparationConstraint(
        vpsc::XDIM,
        m_rectangle_index,
        clusterVarId + 1,
        halfWidth,
...
void RectangularCluster::computeBoundary(const vpsc::Rectangles& rs)
{
    ...
    for (std::set<unsigned>::iterator it = nodes.begin(); it != nodes.end();
         ++it)
    {
...
void RectangularCluster::printCreationCode(FILE* fp) const
{
    ...
    if (!m_margin.empty())
    {
        fprintf(fp, "    cluster%llu->setMargin(", (unsigned long long)this);
        m_margin.outputCode(fp);
        fprintf(fp, ");\n");
    }
    if (!m_padding.empty())
    {
        fprintf(fp, "    cluster%llu->setPadding(", (unsigned long long)this);
...
void Cluster::recPathToCluster(RootCluster* rootCluster, Clusters currentPath)
{
    ...
    for (std::set<unsigned>::iterator it = nodes.begin(); it != nodes.end();
         ++it)
    {
        rootCluster->m_cluster_vectors_leading_to_nodes[*it].push_back(
            currentPath
        );
    }
}
    ...
    for (unsigned i = 0; i < m_cluster_vectors_leading_to_nodes.size(); ++i)
    {
        ...
        for (size_t j = 1; j < paths; ++j)
        {
            for (size_t k = 0; k < j; ++k)
            {
                ...
                size_t   lcaIndex = 0;
                while ((lcaIndex < pathJ.size()) && (lcaIndex < pathK.size())
                ...
                // be a cluster.
                COLA_ASSERT(
                    (lcaIndex < pathJ.size()) || (lcaIndex < pathK.size())
                );
...
{
    for (vector<Cluster*>::iterator i = clusters.begin(); i != clusters.end();
         ++i)
    {
    ...
    if (dim == vpsc::HORIZONTAL)
    {
        ...
        vars.push_back(
            vXMin = new vpsc::Variable(vars.size(), desiredMinX, varWeight)
        );
        vars.push_back(
            vXMax = new vpsc::Variable(vars.size(), desiredMaxX, varWeight)
        );
    }
    else
    {
        ...
        clusterVarId = vars.size();
        vars.push_back(
            vYMin = new vpsc::Variable(vars.size(), desiredMinY, varWeight)
        );
        vars.push_back(
            vYMax = new vpsc::Variable(vars.size(), desiredMaxY, varWeight)
        );
    }
}
