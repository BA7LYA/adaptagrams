...
{
public:
    SparseMatrix(const SparseMap& m)
        : n(m.n)
        , NZ((unsigned)m.nonZeroCount())
        , sparseMap(m)
        , A(std::valarray<double>(NZ))
        , IA(std::valarray<unsigned>(n + 1))
        , JA(std::valarray<unsigned>(NZ))
    {
        unsigned cnt     = 0;
        int      lastrow = -1;
    ...
    void rightMultiply(const std::valarray<double>& v, std::valarray<double>& r)
        const
    {
