...
public:
    ...
    std::set<ShapePair> getExemptPairs(void)
    {
...
public:
    ...
    //! @param group  Assign a group number to this shape. A separation
    //! constraint
    //!               will be generated between two shapes only if they belong
    //!               to the same group. This is useful for clusters.
    //! @param exemptions  Optional set of IDs of shapes to be skipped, i.e.
    //! such that
    //!                    a separation constraint should /not/ be generated
    //!                    between that shape and the one being added.
    void addShape(
        unsigned           id,
        double             halfW,
        double             halfH,
        unsigned int       group      = 1,
        std::set<unsigned> exemptions = std::set<unsigned>()
    );
    ...
    //!          opposite to dim;
    //!      (2) which var is left and which is right in the separation
    //!      constraint. Note however that the dimensions of the boundingBoxes
    //!      do NOT determine the gaps of the separation constraints, which are
    //!      instead based on the half-width and half-height passed when adding
    //!      the shapes.
    void generateSeparationConstraints(
        const vpsc::Dim                dim,
        vpsc::Variables&               vars,
...
private:
    void computeOverlapForShapePairInfo(
        ShapePairInfo&  info,
        vpsc::Variables vs[]
    );
