...
#include "libcola/CompoundConstraint.hxx"
...
void BoundaryConstraint::addShape(const unsigned int index, const double offset)
{
...
std::string BoundaryConstraint::toString(void) const
{
         ...
         ++o)
    {
        ...
        stream << "("
               << "rect: " << info->varIndex << ", offset: " << info->distOffset
               << ")";
        first = false;
    }
    stream << "}";
...
void BoundaryConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
void BoundaryConstraint::generateSeparationConstraints(
    const vpsc::Dim    dim,
    vpsc::Variables&   vars,
    vpsc::Constraints& cs,
    vpsc::Rectangles&  bbs
    ...
    if (dim == _primaryDim)
    {
             ...
             ++o)
        {
            ...
            if (info->distOffset < 0)
            {
                ...
                constraint = new vpsc::Constraint(
                    vars[info->varIndex],
                    variable,
                    -info->distOffset
                );
...
SubConstraintAlternatives BoundaryConstraint::getCurrSubConstraintAlternatives(
    vpsc::Variables vs[]
)
{
    ...
    Offset* info
        = static_cast<Offset*>(_subConstraintInfo[_currSubConstraintIndex]);
...
void AlignmentConstraint::addShape(
...
void AlignmentConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
    if (dim == _primaryDim)
    {
        ...
        if (_isFixed)
        {
            ...
            variable->weight               = 10'0000;
        }
        vars.push_back(variable);
    }
}

void AlignmentConstraint::generateSeparationConstraints(
    const vpsc::Dim    dim,
    vpsc::Variables&   vars,
    vpsc::Constraints& cs,
    vpsc::Rectangles&  bbs
...
SubConstraintAlternatives AlignmentConstraint::getCurrSubConstraintAlternatives(
    vpsc::Variables vs[]
)
{
    SubConstraintAlternatives alternatives;
...
public:
    ...
    {
        return (lConstraint) ? (unsigned)lConstraint->variable->id : varIndex;
    }

    unsigned indexR(void) const
    {
        return (rConstraint) ? (unsigned)rConstraint->variable->id : varIndex2;
    }
...
void SeparationConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
SubConstraintAlternatives SeparationConstraint::
    getCurrSubConstraintAlternatives(vpsc::Variables vs[])
{
    SubConstraintAlternatives alternatives;

    VarIndexPair* info
        = static_cast<VarIndexPair*>(_subConstraintInfo[_currSubConstraintIndex]
        );

    assertValidVariableIndex(vs[_primaryDim], info->indexL());
    assertValidVariableIndex(vs[_primaryDim], info->indexR());
    vpsc::Constraint constraint(
        vs[_primaryDim][info->indexL()],
        vs[_primaryDim][info->indexR()],
        gap,
        equality
    );
    alternatives.push_back(SubConstraint(_primaryDim, constraint));
...
void SeparationConstraint::generateSeparationConstraints(
    const vpsc::Dim    dim,
    vpsc::Variables&   vs,
    vpsc::Constraints& cs,
    vpsc::Rectangles&  bbs
...
{
    VarIndexPair* info = static_cast<VarIndexPair*>(_subConstraintInfo.front());
...
OrthogonalEdgeConstraint::OrthogonalEdgeConstraint(
    const vpsc::Dim dim,
    unsigned        l,
    unsigned        r
)
...
void OrthogonalEdgeConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
SubConstraintAlternatives OrthogonalEdgeConstraint::
    getCurrSubConstraintAlternatives(vpsc::Variables vs[])
{
    COLA_UNUSED(vs);
...
void OrthogonalEdgeConstraint::generateTopologyConstraints(
    const vpsc::Dim                k,
    const vpsc::Rectangles&        rs,
    const vector<vpsc::Variable*>& vars,
...
void OrthogonalEdgeConstraint::rectBounds(
    const vpsc::Dim        k,
    const vpsc::Rectangle* r,
    double&                cmin,
    double&                cmax,
    double&                centre,
    double&                l
...
MultiSeparationConstraint::MultiSeparationConstraint(
    const vpsc::Dim dim,
    double          minSep,
    bool            equality
)
...
void MultiSeparationConstraint::addAlignmentPair(
...
void MultiSeparationConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
SubConstraintAlternatives MultiSeparationConstraint::
    getCurrSubConstraintAlternatives(vpsc::Variables vs[])
{
    COLA_UNUSED(vs);
...
void DistributionConstraint::addAlignmentPair(
    AlignmentConstraint* ac1,
    AlignmentConstraint* ac2
)
{
...
void DistributionConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
SubConstraintAlternatives DistributionConstraint::
    getCurrSubConstraintAlternatives(vpsc::Variables vs[])
{
    COLA_UNUSED(vs);
...
)
{
    ...
    if (dim == _primaryDim)
    {
             ...
             ++o)
        {
            ...
            vpsc::Constraint* c
                = new vpsc::Constraint(c1->variable, c2->variable, sep, true);
            c->creator = this;
            gcs.push_back(c);
            cs.push_back(c);
#if 0
...
{
public:
    RelativeOffset(unsigned indL, unsigned indR, vpsc::Dim dim, double offset)
...
FixedRelativeConstraint::FixedRelativeConstraint(
    const vpsc::Rectangles& rs,
    ...
    , m_shape_vars(shapeIds)
{
         ...
         ++it)
    {
        ...
        {
            unsigned thisId = *it;
            _subConstraintInfo.push_back(new RelativeOffset(
                firstId,
                thisId,
                vpsc::XDIM,
...
void FixedRelativeConstraint::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
SubConstraintAlternatives FixedRelativeConstraint::
    getCurrSubConstraintAlternatives(vpsc::Variables vs[])
{
    COLA_UNUSED(vs);
    ...
    vpsc::Constraint constraint(
        vs[dim][offset->varIndex],
        vs[dim][offset->varIndex2],
        offset->distOffset,
        true
    );
...
)
{
         ...
         ++o)
    {
        ...
        vpsc::Constraint* c = new vpsc::Constraint(
            vars[offset->varIndex],
            vars[offset->varIndex2],
            offset->distOffset,
            true
        );
        c->creator = this;
...
PageBoundaryConstraints::PageBoundaryConstraints(
    double xLow,
    double xHigh,
    double yLow,
    double yHigh,
...
void PageBoundaryConstraints::printCreationCode(FILE* fp) const
{
    fprintf(
        ...
        (unsigned long long)this,
        leftMargin[vpsc::XDIM],
        rightMargin[vpsc::XDIM],
        leftMargin[vpsc::YDIM],
        rightMargin[vpsc::YDIM],
        leftWeight[0]
    );
    for (SubConstraintInfoList::const_iterator o = _subConstraintInfo.begin();
         o != _subConstraintInfo.end();
         ++o)
    {
        ...
        fprintf(
            ...
            (unsigned long long)this,
            offsets->varIndex,
            offsets->halfDim[XDIM],
            offsets->halfDim[YDIM]
        );
    }
    fprintf(
        fp,
...
SubConstraintAlternatives PageBoundaryConstraints::
    getCurrSubConstraintAlternatives(vpsc::Variables vs[])
{
    COLA_UNUSED(vs);
...
void PageBoundaryConstraints::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
    ...
    if (leftWeight[dim])
    {
        vars.push_back(
            vl[dim]
            = new vpsc::Variable(vars.size(), leftMargin[dim], leftWeight[dim])
        );
...
void generateVariablesAndConstraints(
...
{
    for_each(ccs.begin(), ccs.end(), GenerateVariables(dim, vars));
    for_each(
        ccs.begin(),
        ccs.end(),
...
CompoundConstraint::~CompoundConstraint()
{
    ...
    for_each(
        _subConstraintInfo.begin(),
        _subConstraintInfo.end(),
        delete_object()
    );
...
void CompoundConstraint::updateVarIDsWithMapping(
    const VariableIDMap& idMap,
    bool                 forward
)
{
...
void CompoundConstraint::assertValidVariableIndex(
...
bool VariableIDMap::addMappingForVariable(
    const unsigned from,
    const unsigned to
)
{
...
unsigned VariableIDMap::mappingForVariable(const unsigned var, bool forward)
    const
{
    for (IDPairList::const_iterator it = m_mapping.begin();
         it != m_mapping.end();
