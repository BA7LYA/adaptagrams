///
/// @file ConstrainedMajorizationLayout.cxx
...
ConstrainedMajorizationLayout ::ConstrainedMajorizationLayout(
    vector<Rectangle*>& rs,
    ...
    , Y(valarray<double>(n))
    , stickyNodes(false)
    , startX(valarray<double>(n))
    , startY(valarray<double>(n))
    ...
    , gpY(nullptr)
    , ccs(nullptr)
    , unsatisfiableX(nullptr)
    , unsatisfiableY(nullptr)
    , avoidOverlaps(None)
    ...
    , majorization(true)
{
    ...
    if (clusterHierarchy)
    {
        ...
        {
            Cluster* c = *i;
            for (set<unsigned>::iterator j = c->nodes.begin();
                 j != c->nodes.end();
                 j++)
            {
                for (set<unsigned>::iterator k = c->nodes.begin();
                     k != c->nodes.end();
                     k++)
                {
                    unsigned a = *j, b = *k;
...
void ConstrainedMajorizationLayout::setStickyNodes(
    ...
    const valarray<double>& startX,
    const valarray<double>& startY
)
{
...
void ConstrainedMajorizationLayout::majorize(
    const valarray<double>& Dij,
    GradientProjection*     gp,
    valarray<double>&       coords,
    const valarray<double>& startCoords
)
{
...
inline double ConstrainedMajorizationLayout ::compute_stress(
    const valarray<double>& Dij
)
{
    double sum = 0, d, diff;
...
void ConstrainedMajorizationLayout::run(bool x, bool y)
{
    if (constrainedLayout)
    {
        vector<vpsc::Rectangle*>* pbb
            = boundingBoxes.empty() ? nullptr : &boundingBoxes;
    ...
    if (n > 0)
    {
        do {
            ...
            // could create cluster boundaries here with chains of dummy nodes
            // (a dummy node for each vertex of the convex hull) connected by
            // dummy straightenEdges and we'd then continue on to
            // straightenEdges below. This should work assuming we already have
            // a feasible (i.e. non overlapping cluster) state.  The former
            // could be enforced by an earlier stage involving simple
            ...
            else
            {
                if (majorization)
                {
                    if (x)
                    {
                        majorize(Dij, gpX, X, startX);
                    }
                    if (y)
                    {
                        majorize(Dij, gpY, Y, startY);
                    }
                }
                else
                {
                    if (x)
                    {
                        newton(Dij, gpX, X, startX);
                    }
                    if (y)
                    {
                        newton(Dij, gpY, Y, startY);
                    }
                }
            }
            if (clusterHierarchy)
            {
                for (Clusters::iterator c = clusterHierarchy->clusters.begin();
                     c != clusterHierarchy->clusters.end();
                     c++)
                {
...
void ConstrainedMajorizationLayout::straighten(
...
)
{
    ...
    for (unsigned i = 0; i < sedges.size(); i++)
    {
        ...
        for (unsigned j = 1; j < activePath.size(); j++)
        {
            ...
            {
                unsigned b = path[k];
                // might be useful to have greater weight for potential bends
                // than actual bends
        ...
        {
            unsigned u = path[activePath[j - 1]], b = path[activePath[j]],
    ...
    // std::cout << (dim==HORIZONTAL?"X":"Y") << " snodes.size
    // "<<snodes.size()<< " n="<<n<<std::endl; std::cout << "Generated
    // "<<linearConstraints.size()<< " linear constraints"<<std::endl;
    SparseMap Q(snodes.size());
    for (straightener::LinearConstraints::iterator i
         = linearConstraints.begin();
         i != linearConstraints.end();
         i++)
    {
    ...
    const valarray<double>& r = gp->getFullResult();
