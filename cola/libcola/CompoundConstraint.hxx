...
#ifndef SWIG
using Constraints = std::vector<vpsc::Constraint*>;
using Variables   = std::vector<vpsc::Variable*>;
#endif
...
{
public:
    SubConstraint(vpsc::Dim dim, vpsc::Constraint constraint, double cost = 0)
        : dim(dim)
...
using SubConstraintAlternatives = std::list<SubConstraint>;

static const double freeWeight = 0.0001;

static const unsigned int DEFAULT_CONSTRAINT_PRIORITY = 3'0000;
...
public:
    ...
    virtual void updateVarIDsWithMapping(
...
using SubConstraintInfoList = std::vector<SubConstraintInfo*>;
...
public:
     ...
     *                      be appended.
     */
    virtual void generateVariables(const vpsc::Dim dim, vpsc::Variables& vars)
        = 0;
...
public:
    ...
    void generateTopologyConstraints(
        const vpsc::Dim                     k,
        const vpsc::Rectangles&             rs,
        const std::vector<vpsc::Variable*>& vars,
...
using UnsatisfiableConstraintInfos = std::vector<UnsatisfiableConstraintInfo*>;
