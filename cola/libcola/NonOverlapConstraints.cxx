...
#include "libcola/NonOverlapConstraints.hxx"
...
bool NonOverlapConstraintExemptions::shapePairIsExempt(ShapePair shapePair
) const
{
...
NonOverlapConstraints::NonOverlapConstraints(
    NonOverlapConstraintExemptions* exemptions,
...
void NonOverlapConstraints::removeShape(unsigned id)
{
    ...
    // Remove all ShapePairInfo objects having this id as one of their two
    // indices.
    ...
    while (it != pairInfoList.end())
    {
        ...
        {
            it = pairInfoList.erase(it
            );  // now it points to next list element after one erased
        }
        else
        {
            it++;
...
void NonOverlapConstraints::generateVariables(
    const vpsc::Dim  dim,
    vpsc::Variables& vars
)
{
...
void NonOverlapConstraints::computeOverlapForShapePairInfo(
    ShapePairInfo&  info,
    vpsc::Variables vs[]
)
{
    ...
    else
    {
        ...
        double penalty = 10'0000;
        if ((left1 >= left2) && (right1 <= right2) && (bottom1 >= bottom2)
            && (top1 <= top2))
        {
        ...
        else if ((left2 >= left1) && (right2 <= right1) && (bottom2 >= bottom1) && (top2 <= top1))
        {
...
SubConstraintAlternatives NonOverlapConstraints::
...
)
{
         ...
         ++info)
    {
        ...
        vpsc::Rectangle rect1 = (shape1.cluster)
                                  ? shape1.cluster->bounds
                                  : *boundingBoxes[info->varIndex1];
        vpsc::Rectangle rect2 = (shape2.cluster)
                                  ? shape2.cluster->bounds
                                  : *boundingBoxes[info->varIndex2];
