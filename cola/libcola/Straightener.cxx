...
#include "libcola/Straightener.hxx"
...
void Route::rerouteAround(vpsc::Rectangle* rect)
{
    ...
    for (unsigned i = 1; i < n - 1; i++)
    {
        ...
        if (rect->inside(x, y))
        {
            ...
            switch (projectSide)
            {
            case LEFT: x = rect->getMinX(); break;
            case BOTTOM: y = rect->getMinY(); break;
            case RIGHT: x = rect->getMaxX(); break;
            case TOP: y = rect->getMaxY(); break;
            }
        }
    }
    ...
    {
        // we have projected all points to the boundary already so we shouldn't
        // find any inside
        COLA_ASSERT(!rect->inside(xs[i], ys[i]));
        vpsc::RectangleIntersections ri;
        rect->lineIntersections(prevX, prevY, xs[i], ys[i], ri);
        if (ri.intersects)
        {
            int count = ri.countIntersections();
            COLA_ASSERT(
                count > 0
            );  // can't be 0 because we have detected an intersection
            COLA_ASSERT(
                count < 4
            );  // assumes no zero width or height rects which would be
            ...
            {
                // single intersection, earlier projection step ensures it is on
                // the perimeter, so nothing to do
            }
        }
        prevX = xs[i];
        prevY = ys[i];
...
void Edge::nodePath(vector<Node*>& nodes, bool allActive = true)
{
    ...
    for (unsigned i = 1; i < route->n; i++)
    {
        ...
        for (list<unsigned>::iterator j = ds.begin(); j != ds.end();)
        {
            ...
            // printf("     px=%f, py=%f, ax=%f, ay=%f, bx=%f,
            // by=%f\n",px,py,ax,ay,bx,by);
        ...
        for (set<pair<double, unsigned>>::iterator j
             = pntsOnLineSegment.begin();
             j != pntsOnLineSegment.end();
             j++)
        {
...
void Edge::createRouteFromPath(const std::vector<Node*>& nodes)
{
...
struct Event
{
    ...
    Event(EventType t, Node* v, double p)
        ...
        , e(nullptr)
        , pos(p) {};
    Event(EventType t, Edge* e, double p)
        : type(t)
        , v(nullptr)
        , e(e)
        , pos(p) {};
 ...
 *   transitivity:  CompareEvents(a,b) && CompareEvents(b,c) =>
 * CompareEvents(a,c) transitivity of equivalence: !CompareEvents(a,b) &&
 * !CompareEvents(b,a) && !CompareEvents(b,c) && !CompareEvents(c,b)
...
void sortNeighbours(
    ...
    Node*                l,
    Node*                r,
    const double         conjpos,
    const vector<Edge*>& openEdges,
    vector<Node*>&       L,
    vector<Node*>&       nodes
)
{
    ...
    for (unsigned i = 0; i < openEdges.size(); i++)
    {
        ...
        // std::cerr <<
        // "edge(intersections="<<bs.size()<<":("<<e->startNode<<","<<e->endNode<<"))"<<std::endl;
        for (vector<double>::iterator it = bs.begin(); it != bs.end(); it++)
        {
            sortedEdges.insert(make_pair(*it, e));
        }
    }
    for (set<PosEdgePair>::iterator i = sortedEdges.begin();
         i != sortedEdges.end();
         i++)
    {
        double pos = i->first;
        ...
        Edge* e = i->second;
        if (e->startNode == v->id || e->endNode == v->id)
        {
            continue;
        }
        // if(l!=nullptr&&(e->startNode==l->id||e->endNode==l->id)) continue;
        // cerr <<
        // "edge("<<e->startNode<<","<<e->endNode<<",pts="<<e->pts<<")"<<endl;
...
static double overlap(vpsc::Dim k, const Node* u, const Node* v)
{
...
#ifdef STRAIGHTENER_DEBUG
    cout
        << (dim == vpsc::HORIZONTAL ? "scanning top to bottom..."
    ...
    for (unsigned i = 0; i < events.size(); i++)
    {
        ...
        if (v != nullptr)
        {
            ...
            if (!openNodes.empty())
            {
                ...
                {
                    if (!xSkipping || dim != vpsc::HORIZONTAL
                        || overlap(vpsc::HORIZONTAL, *it, v) <= 0
                        || overlap(vpsc::HORIZONTAL, *it, v)
                               <= overlap(vpsc::VERTICAL, *it, v))
                    {
                        l = *it;
                        break;
                    }
                ...
                while (it != openNodes.end())
                {
                    if (!xSkipping || dim != vpsc::HORIZONTAL
                        ...
                        || overlap(vpsc::HORIZONTAL, v, *it)
                               <= overlap(vpsc::VERTICAL, v, *it))
                    {
                        r = *it;
                        break;
                    }
            ...
            if (e->type == Close)
            {
                if (l != nullptr)
                {
                    cs.push_back(createConstraint(l, v, dim));
                }
                if (r != nullptr)
                {
                    cs.push_back(createConstraint(v, r, dim));
                }
            }
        }
        if (e->type == Open)
...
void generateClusterBoundaries(
    ...
    const cola::Cluster&            clusterHierarchy,
...
)
{
    ...
    for (vector<cola::Cluster*>::const_iterator i
         ...
         i != clusterHierarchy.clusters.end();
         i++)
    {
...
Straightener::Straightener(
    ...
    const cola::FixedList&               fixed,
    const std::vector<Edge*>&            edges,
    const vpsc::Variables&               vs,
    vpsc::Variables&                     lvs,
    ...
    , lvs(lvs)
{
    ...
    // after generateConstraints we have new dummy nodes at the end of snodes
    // and constraints in cs
    ...
    for (vector<cola::SeparationConstraint*>::iterator i = cs.begin();
         i != cs.end();
         i++)
    {
        unsigned        lv  = (*i)->left();
...
double Straightener::computeStress(const std::valarray<double>& coords)
{
    double stress = 0;
    for (unsigned i = 0; i < edges.size(); i++)
    {
...
double Straightener::computeStress2(const std::valarray<double>& coords)
{
    COLA_UNUSED(coords);
...
double pathLength(const Edge* e, const vector<Node*>& nodes)
{
    double                  length = 0;
    const vector<unsigned>& path   = e->path;
