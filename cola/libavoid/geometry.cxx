...
)
{
    ...
    else if ((((s2 == e1) || pointOnLine(s1, s2, e1)) && (vecDir(s1, s2, e2) != 0)) || (((s2 == e2) || pointOnLine(s1, s2, e2)) && (vecDir(s1, s2, e1) != 0)))
    {
...
double angle(const Point& a, const Point& b, const Point& c)
{
    double ux = b.x - a.x, uy = b.y - a.y, vx = c.x - b.x, vy = c.y - b.y,
           lu = sqrt(ux * ux + uy * uy), lv = sqrt(vx * vx + vy * vy),
           udotv = ux * vx + uy * vy, costheta = udotv / (lu * lv);
...
bool inPolyGen(const PolygonInterface& argpoly, const Point& q)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        if ((P[i].y > 0) != (P[i1].y > 0))
        {
            ...
            double x
                = (P[i].x * P[i1].y - P[i1].x * P[i].y) / (P[i1].y - P[i].y);
