...
)
{
    ...
    else
    {
             ...
             k          = k->lstNext)
        {
            ...
            else if (k->id.isConnPt() && !k->id.isConnectionPin() && !(k->id.isConnCheckpoint() && k->id.objID == pID.objID))
            {
...
static void vertexSweep(VertInf* vert)
{
    ...
    for (VertInf* inf = beginVert; inf != endVert; inf = inf->lstNext)
    {
            ...
            && !inf->id.isConnPt())
        {
            ...
            db_printf(
                "Center is inside shape %u so ignore shape edges.\n",
                shapeID
            );
            continue;
        }
    ...
    for (VertSet::const_iterator t = vbegin; t != vend; ++t)
    {
        ...
        if (!(centerID.isConnPt()))
        {
            cone1 = inValidRegion(
                ...
                centerInf->shPrev->point,
                centerPoint,
                centerInf->shNext->point,
                currInf->point
            );
        }
        if (!(currInf->id.isConnPt()))
        {
            cone2 = inValidRegion(
                router->IgnoreRegions,
                currInf->shPrev->point,
                currInf->point,
                currInf->shNext->point,
                centerPoint
            );
        }
