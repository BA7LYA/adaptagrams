...
{
    COLA_ASSERT(
        (type == ShapeAdd) || (type == ShapeRemove) || (type == ShapeMove)
    );
}
...
{
    COLA_ASSERT(
        (type == ShapeMove) || (type == ShapeAdd) || (type == ShapeRemove)
        || (type == JunctionMove) || (type == JunctionAdd)
        || (type == JunctionRemove)
    );
...
void ActionInfo::addConnEndUpdate(
...
)
{
    ...
    for (ConnUpdateList::iterator conn = conns.begin(); conn != conns.end();
         ++conn)
    {
