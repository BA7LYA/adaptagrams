...
#include "libavoid/JunctionRef.hxx"
    ...
    , m_position_fixed(false)
{
    ...
    ShapeConnectionPin* pin = new Avoid::ShapeConnectionPin(
        this,
        Avoid::CONNECTIONPIN_CENTRE,
        ConnDirAll
    );
...
JunctionRef::~JunctionRef()
{
    ...
    {
        err_printf(
            "ERROR: JunctionRef::~JunctionRef() shouldn't be called directly.\n"
        );
        err_printf(
            "       It is owned by the router.  Call Router::deleteJunction() "
            "instead.\n"
        );
        abort();
    }
...
void JunctionRef::outputCode(FILE* fp) const
{
    fprintf(
        fp,
        "    JunctionRef *junctionRef%u = new JunctionRef(router, "
        "Point(%g, %g), %u);\n",
        id(),
        position().x,
        position().y,
