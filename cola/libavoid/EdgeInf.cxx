...
void EdgeInf::makeActive(void)
{
    ...
    if (m_orthogonal)
    {
        ...
        m_pos1 = m_vert1->orthogVisList.insert(
            m_vert1->orthogVisList.begin(),
            this
        );
        m_vert1->orthogVisListSize++;
        m_pos2 = m_vert2->orthogVisList.insert(
            m_vert2->orthogVisList.begin(),
            this
        );
    ...
    else
    {
        ...
        else  // if (invisible)
        {
            ...
            m_pos1
                = m_vert1->invisList.insert(m_vert1->invisList.begin(), this);
            m_vert1->invisListSize++;
            m_pos2
                = m_vert2->invisList.insert(m_vert2->invisList.begin(), this);
    ...
    for (VertInf* k = m_router->vertices.shapesBegin(); k != last;)
    {
        ...
        if (kID.objID != lastId)
        {
            if ((ss.find(kID.objID) != ss.end()))
            {
                ...
                db_printf(
                    "Endpoint is inside shape %u so ignore shape "
                    "edges.\n",
                    kID.objID
                );
...
{
    return (
        (m_vert1->point.x == m_vert2->point.x)
...
bool EdgeInf::isDummyConnection(void) const
{
    ...
    return (
        (m_vert1->id.isConnectionPin() && m_vert2->id.isConnPt())
