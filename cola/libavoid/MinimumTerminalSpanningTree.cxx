///
/// @file MinimumTerminalSpanningTree.cxx
...
#include "libavoid/MinimumTerminalSpanningTree.hxx"
...
{
    for (VertexSetList::iterator it = allsets.begin(); it != allsets.end();
         ++it)
    {
...
void MinimumTerminalSpanningTree::unionSets(
    VertexSetList::iterator s1,
...
HyperedgeTreeNode* MinimumTerminalSpanningTree::addNode(
    VertInf*           vertex,
...
void MinimumTerminalSpanningTree::buildHyperedgeTreeToRoot(
    VertInf*           currVert,
    HyperedgeTreeNode* prevNode,
    VertInf*           prevVert,
    bool               markEdges
...
VertInf** MinimumTerminalSpanningTree::resetDistsForPath(
    VertInf*  currVert,
...
void MinimumTerminalSpanningTree::constructSequential(void)
{
    ...
    while (!vHeap.empty())
    {
             ...
             ++edge)
        {
            ...
            COLA_ASSERT(
                !freeConnection
                == (u->pathNext
                    && !colinear(u->pathNext->point, u->point, v->point))
            );
            if (!freeConnection)
            {
            ...
            else
            {
                ...
                double secondJoinCost
                    = connectsWithoutBend(v, u) ? 0.0 : bendPenalty;
    ...
    while (!beHeap.empty())
    {
        ...
        if (s1 != s2)
        {
            ...
            {
                router->debugHandler()
                    ->mtstCommitToEdge(e->m_vert1, e->m_vert2, true);
            }
#endif
...
VertInf* MinimumTerminalSpanningTree::orthogonalPartner(
    VertInf* vert,
    double   penalty
)
{
    ...
    {
        vert->m_orthogonalPartner
            = new VertInf(router, dimensionChangeVertexID, vert->point, false);
        vert->m_orthogonalPartner->m_orthogonalPartner = vert;
        extraVertices.push_back(vert->m_orthogonalPartner);
        EdgeInf* extraEdge
            = new EdgeInf(vert->m_orthogonalPartner, vert, isOrthogonal);
...
void MinimumTerminalSpanningTree::removeInvalidBridgingEdges()
{
    ...
    for (size_t i = 0; i < beHeapSize; ++i)
    {
        ...
        bool       valid = (ends.first->treeRoot() != ends.second->treeRoot())
                  ...
                  && (origTerminals.find(ends.first->treeRoot())
                      != origTerminals.end())
                  && (origTerminals.find(ends.second->treeRoot())
                      != origTerminals.end());
        if (!valid)
        {
    ...
    getOrthogonalEdgesFromVertex(VertInf* vert, VertInf* prev)
{
    ...
    EdgeInfList& visList
        = (!isOrthogonal) ? realVert->visList : realVert->orthogVisList;
    EdgeInfList::const_iterator finish = visList.end();
    for (EdgeInfList::const_iterator edge = visList.begin(); edge != finish;
         ++edge)
    {
...
void MinimumTerminalSpanningTree::constructInterleaved(void)
{
    ...
    while (!vHeap.empty())
    {
        ...
        if (!beHeap.empty()
            && u->sptfDist >= (0.5 * beHeap.front()->mtstDist()))
        {
            // Take the lowest cost edge.
            EdgeInf* e = beHeap.front();
        ...
        // For each edge from this vertex...
        LayeredOrthogonalEdgeList edgeList
            = getOrthogonalEdgesFromVertex(u, u->pathNext);
             ...
             ++edge)
        {
            ...
            else
            {
                ...
                bool   found = std::find(beHeap.begin(), beHeap.end(), e)
                          != beHeap.end();
                if (!found)
                {
...
bool MinimumTerminalSpanningTree::connectsWithoutBend(
    VertInf* oldLeaf,
    VertInf* newLeaf
)
{
    ...
    if (oldLeaf->sptfDist == 0)
    {
        ...
        EdgeInfList& visList
            = (!isOrthogonal) ? oldLeaf->visList : oldLeaf->orthogVisList;
        EdgeInfList::const_iterator finish = visList.end();
        for (EdgeInfList::const_iterator edge = visList.begin(); edge != finish;
             ++edge)
        {
...
void MinimumTerminalSpanningTree::rewriteRestOfHyperedge(
    VertInf*  vert,
...
)
{
    ...
    LayeredOrthogonalEdgeList edgeList
        = getOrthogonalEdgesFromVertex(vert, nullptr);
    for (LayeredOrthogonalEdgeList::const_iterator edge = edgeList.begin();
         edge != edgeList.end();
         ++edge)
    {
...
void MinimumTerminalSpanningTree::drawForest(VertInf* vert, VertInf* prev)
{
    if (prev == nullptr)
    {
        ...
        */
        ...
        // fprintf(debug_fp, "<circle cx=\"%g\" cy=\"%g\" r=\"3\"
        // db:sptfDist=\"%g\" "
        //         "style=\"fill: %s; stroke: %s; fill-opacity: 0.5; "
        //         "stroke-width: 1px; stroke-opacity:0.5\" />\n",
        //         vert->point.x, vert->point.y, vert->sptfDist, colour.c_str(),
        //         "black");
    }
         ...
         ++edge)
    {
        ...
        if (v->treeRoot() == vert->treeRoot())
        {
            if (v->pathNext == vert)
            {
                ...
                {
                    router->debugHandler()
                        ->mtstGrowForestWithEdge(vert, v, false);
                }
                drawForest(v, vert);
            }
        }
...
VertexPair MinimumTerminalSpanningTree::realVerticesCountingPartners(
    EdgeInf* edge
)
{
    ...
    if ((v1->id != dimensionChangeVertexID)
        && (v2->id != dimensionChangeVertexID) && (v1->point != v2->point)
...
void MinimumTerminalSpanningTree::commitToBridgingEdge(EdgeInf* e)
{
    VertexPair ends = realVerticesCountingPartners(e);
    VertInf*   newRoot
        = std::min(ends.first->treeRoot(), ends.second->treeRoot());
    VertInf* oldRoot
        = std::max(ends.first->treeRoot(), ends.second->treeRoot());
