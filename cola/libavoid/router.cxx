...
void Router::addShape(ShapeRef* shape)
{
    ...
    COLA_ASSERT(
        find(
            actionList.begin(),
            actionList.end(),
            ActionInfo(ShapeRemove, shape)
        )
        == actionList.end()
    );
    COLA_ASSERT(
        find(actionList.begin(), actionList.end(), ActionInfo(ShapeMove, shape))
        == actionList.end()
    );
...
void Router::deleteShape(ShapeRef* shape)
{
    ...
    // XXX: Possibly we could handle this by ordering them intelligently.
    COLA_ASSERT(
        find(actionList.begin(), actionList.end(), ActionInfo(ShapeAdd, shape))
        == actionList.end()
    );

    // Delete any ShapeMove entries for this shape in the action list.
    ActionInfoList::iterator found = find(
        actionList.begin(),
        actionList.end(),
        ActionInfo(ShapeMove, shape)
    );
    if (found != actionList.end())
    {
...
void Router::processActions(void)
{
    ...
    for (curr = actionList.begin(); curr != finish; ++curr)
    {
        ...
        bool         isMove
            = (actInf.type == ShapeMove) || (actInf.type == JunctionMove);
        ;
    ...
    if (seenShapeMovesOrDeletes && m_allows_polyline_routing)
    {
        if (InvisibilityGrph)
        {
            ...
            for (std::list<unsigned int>::iterator it
                 = deletedObstacles.begin();
                 it != deletedObstacles.end();
                 ++it)
            {
...
void Router::addJunction(JunctionRef* junction)
{
    ...
    COLA_ASSERT(
        find(
            actionList.begin(),
            actionList.end(),
            ActionInfo(JunctionRemove, junction)
        )
        == actionList.end()
    );
    COLA_ASSERT(
        find(
            actionList.begin(),
            actionList.end(),
            ActionInfo(JunctionMove, junction)
        )
        == actionList.end()
    );
...
void Router::deleteJunction(JunctionRef* junction)
{
    ...
    COLA_ASSERT(
        find(
            actionList.begin(),
            actionList.end(),
            ActionInfo(JunctionAdd, junction)
        )
        == actionList.end()
    );
...
bool Router::objectIdIsUnused(const unsigned int id) const
{
    ...
    // Examine connectors.
    for (ConnRefList::const_iterator i = connRefs.begin(); i != connRefs.end();
         ++i)
    {
        if ((*i)->id() == id)
        {
...
void Router::rerouteAndCallbackConnectors(void)
{
    ...
    // generated here.
    ConnRefSet hyperedgeConns = m_hyperedge_rerouter.calcHyperedgeConnectors();
    ...
    bool withMinorImprovements
        = routingOption(improveHyperedgeRoutesMovingJunctions);
    bool withMajorImprovements
    ...
    for (ConnRefList::const_iterator i = reroutedConns.begin(); i != fin; ++i)
    {
        ...
        ConnRefList::iterator findIt
            = std::find(deletedConns.begin(), deletedConns.end(), conn);
...
static double cheapEstimatedCost(ConnRef* lineRef)
{
    ...
    for (size_t i = 1; i < route.size(); ++i)
    {
        ...
        double segmentLength
            = (isPolyLine) ? euclideanDist(a, b) : manhattanDist(a, b);
    ...
    ConnCostRefSetList crossingSetsListToRemoveCrossingsFromGroups(void)
    {
             ...
             ++it)
        {
            ...
            while ((crossingCausingConnector
                    = removeConnectorWithMostCrossings(pairsSet))
                       .second
                   != nullptr)
            {
            ...
            for (CrossingConnectorsMap::const_iterator it2 = pairsSet.begin();
                 it2 != pairsSet.end();
                 ++it2)
            {
...
private:
    ...
    // crossing count.
    ConnCostRef removeConnectorWithMostCrossings(CrossingConnectorsMap& pairsSet
    )
    {
        ...
        return std::make_pair(
            static_cast<double>(candidateCrossingCount),
            candidateConnector
        );
    }
...
void Router::performContinuationCheck(
...
)
{
    ...
    unsigned int elapsedMsec
        = (unsigned int)((clock() - m_transaction_start_time)
                         / (CLOCKS_PER_SEC / (double)1000));

    bool shouldContinue = shouldContinueTransactionWithProgress(
        elapsedMsec,
        phaseNumber,
...
bool Router::shouldContinueTransactionWithProgress(
...
void Router::newBlockingShape(const Polygon& poly, int pid)
{
    ...
    for (EdgeInf* iter = visGraph.begin(); iter != finish;)
    {
        ...
        if (tmp->getDist() != 0)
        {
            ...
            bool ep_in_poly1
                = (eID1.isConnPt()) ? inPoly(poly, e1, countBorder) : false;
            bool ep_in_poly2
                = (eID2.isConnPt()) ? inPoly(poly, e2, countBorder) : false;
...
void Router::generateContains(VertInf* pt)
{
    ...
    for (ClusterRefList::const_iterator i = clusterRefs.begin(); i != clFinish;
         ++i)
    {
...
void Router::adjustClustersWithAdd(
...
static double AngleAFromThreeSides(
    const double a,
    const double b,
    const double c
)
...
void Router::setRoutingParameter(
...
)
{
    ...
    if (value < 0)
    {
        ...
        switch (parameter)
        {
        case segmentPenalty: m_routing_parameters[parameter] = 50; break;
        case fixedSharedPathPenalty:
            m_routing_parameters[parameter] = 110;
            break;
        case anglePenalty: m_routing_parameters[parameter] = 50; break;
        case crossingPenalty: m_routing_parameters[parameter] = 200; break;
        case clusterCrossingPenalty:
            m_routing_parameters[parameter] = 4000;
            break;
        case idealNudgingDistance: m_routing_parameters[parameter] = 4.0; break;
        case portDirectionPenalty: m_routing_parameters[parameter] = 100; break;
        default: m_routing_parameters[parameter] = 50; break;
        }
    }
    else
    {
...
void Router::printInfo(void)
{
    ...
    for (EdgeInf* t = visGraph.begin(); t != visGraph.end(); t = t->lstNext)
    {
    ...
    for (EdgeInf* t = invisGraph.begin(); t != invisGraph.end(); t = t->lstNext)
    {
        st_invalid_visedges++;
    }
    ...
    fprintf(
        ...
        "%d invalid)\n",
        st_valid_shape_visedges + st_invalid_visedges + st_valid_endpt_visedges,
        st_valid_shape_visedges + st_valid_endpt_visedges,
        st_valid_shape_visedges,
        st_valid_endpt_visedges,
        st_invalid_visedges
    );
...
void Router::outputInstanceToSVG(std::string instanceName)
{
    ...
    for (size_t p = 0; p < lastRoutingOptionMarker; ++p)
    {
        fprintf(
            ...
            (unsigned long)p,
            (m_routing_options[p]) ? "true" : "false"
        );
    }
    ...
    while (obstacleIt != m_obstacles.end())
    {
        ...
        for (size_t i = 0; i < obstacle->polygon().size(); ++i)
        {
            fprintf(
                ...
                "%c %g %g ",
                ((i == 0) ? 'M' : 'L'),
                obstacle->polygon().at(i).x,
                obstacle->polygon().at(i).y
            );
        }
        fprintf(fp, "Z\" />\n");
        ++obstacleIt;
         ...
         ++obstacleIt)
    {
        ...
        if (junction)
        {
            fprintf(
                ...
                junction->recommendedPosition().y,
                "green"
            );
        }
    }
    ...
    for (EdgeInf* t = visGraph.begin(); t != finish; t = t->lstNext)
    {
        ...
        fprintf(
            ...
            p2.x,
            p2.y,
            (ids.first.isConnPt() || ids.second.isConnPt()) ? "blue" : "red"
        );
    }
    fprintf(fp, "</g>\n");
    fprintf(
    ...
    for (EdgeInf* t = visOrthogGraph.begin(); t != finish; t = t->lstNext)
    {
        ...
        fprintf(
            ...
            p2.x,
            p2.y,
            (ids.first.isConnPt() || ids.second.isConnPt()) ? "green" : "red"
        );
    }
    fprintf(fp, "</g>\n");
...
void Router::outputDiagramSVG(std::string instanceName, LineReps* lineReps)
{
    ...
    if (lineReps)
    {
        for (LineReps::iterator it = lineReps->begin(); it != lineReps->end();
             ++it)
        {
            fprintf(fp, "<path d=\"M %g %g ", it->begin.x, it->begin.y);
...
HyperedgeNewAndDeletedObjectLists Router::
