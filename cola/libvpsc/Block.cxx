...
void Block::setUpConstraintHeap(
    PairingHeap<Constraint*, CompareConstraints>*& h,
    bool                                           in
)
{
    delete h;
...
Constraint* Block::findMinInConstraint()
{
    ...
    while (!in->isEmpty())
    {
        ...
        f << "    timestamps: left=" << lb->timeStamp
          << " right=" << rb->timeStamp << " constraint=" << v->timeStamp
          << endl;
#endif
        if (lb == rb)
        {
...
// variable u.
void Block::populateSplitBlock(Block* b, Variable* v, const Variable* u)
{
    b->addVariable(v);
 ...
 */
bool Block::getActivePathBetween(
    Constraints&    path,
    const Variable* u,
    const Variable* v,
    const Variable* w
...
bool Block::isActiveDirectedPathBetween(const Variable* u, const Variable* v)
    const
{
    if (u == v)
    {
