...
public:
     ...
     */
    double getMinD(const unsigned d) const
    {
        COLA_ASSERT(d == 0 || d == 1);
     ...
     * @param d axis: 0=horizontal 1=vertical
     */
    double getMaxD(const unsigned d) const
    {
        COLA_ASSERT(d == 0 || d == 1);
        return (d == 0 ? getMaxX() : getMaxY());
    }

    void setMinD(const unsigned d, const double val)
    {
        if (d == 0)
        {
            minX = val;
    ...
    void setMaxD(const unsigned d, const double val)
    {
        if (d == 0)
        {
            maxX = val;
     ...
     * @param d axis: 0=horizontal 1=vertical
     */
    double getCentreD(const unsigned d) const
    {
        COLA_ASSERT(d == 0 || d == 1);
     ...
     */
    double length(const unsigned d) const
    {
        COLA_ASSERT(d == 0 || d == 1);
    ...
    {
        maxX = minX + w - 2.0 * xBorder;
    }

    void set_height(double h)
    {
        maxY = minY + h - 2.0 * yBorder;
    }
 ...
 *
 * @param[in,out] rs    The rectangles which will be moved to remove overlap
 * @param[in] fixed     A set of indices to rectangles which should not be
 * moved.
 * @param[in] thirdPass Optionally run the third horizontal pass described
 * above.
 */
void removeoverlaps(
    Rectangles&               rs,
