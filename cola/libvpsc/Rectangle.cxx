...
#include "libvpsc/Constraint.hxx"
#include "libvpsc/LineSegment.hxx"
...
{
    os << "Hue[0.17],Rectangle[{" << r.getMinX() << "," << r.getMinY() << "},{"
       << r.getMaxX() << "," << r.getMaxY() << "}]";
    return os;
}
...
struct Node
{
    ...
    Node(Variable* v, Rectangle* r, double p)
        ...
        , firstAbove(nullptr)
        , firstBelow(nullptr)
        , leftNeighbours(nullptr)
        , rightNeighbours(nullptr)
...
)
{
    ...
    case LineSegment::NOT_INTERSECTING: return true;
...
void Rectangle::lineIntersections(
    double                  x1,
    double                  y1,
    double                  x2,
    double                  y2,
...
) const
{
    ...
    LineSegment bottom(
        Vector(getMinX(), getMinY()),
        Vector(getMaxX(), getMinY())
    );
    if (!checkIntersection(
            l.Intersect(bottom, intersection),
            intersection,
            ri,
            ri.bottom,
            ri.bottomX,
    ...
    LineSegment left(
        Vector(getMinX(), getMinY()),
        Vector(getMinX(), getMaxY())
    );
    if (!checkIntersection(
            l.Intersect(left, intersection),
            intersection,
            ri,
            ri.left,
            ri.leftX,
    ...
    LineSegment right(
        Vector(getMaxX(), getMinY()),
        Vector(getMaxX(), getMaxY())
    );
    if (!checkIntersection(
            l.Intersect(right, intersection),
            intersection,
            ri,
            ri.right,
            ri.rightX,
...
)
{
    ...
    xs.push_back(x1);
    ys.push_back(y1);
    bool top1 = eq(y1, maxY), top2 = eq(y2, maxY), bottom1 = eq(y1, minY),
         bottom2 = eq(y2, minY);
    bool left1 = eq(x1, minX), left2 = eq(x2, minX), right1 = eq(x1, maxX),
         right2      = eq(x2, maxX);
 ...
 * vertical pass) can be applied wherein the x-positions of rectangles are reset
 * to their original positions and overlap removal repeated.  This may avoid
