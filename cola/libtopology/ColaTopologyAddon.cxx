...
#include "libtopology/ColaTopologyAddon.hxx"
...
void ColaTopologyAddon::handleResizes(
...
)
{
    ...
    ResizeMap resizes;
    for (cola::Resizes::const_iterator r = resizeList.begin();
         r != resizeList.end();
         ++r)
    {
...
void ColaTopologyAddon::writeSVGFile(std::string basename)
{
    ...
    for (size_t i = 0; i < topologyRoutes.size(); ++i)
    {
        ...
        fprintf(fp, "<path id=\"edge-%u\" class=\"edge\" d=\"", (unsigned)i);
...
void ColaTopologyAddon::makeFeasible(
             ...
             ++i, ++clusterCount)
        {
            ...
            if (c != nullptr)
            {
                double idealCircumference
                    = 2.0 * sqrt(M_PI * c->area(boundingBoxes));
                ...
                eps.push_back(eps[0]);
                // cout << "addToPath(vs[" << eps[0]->node->id <<
                // "],(EdgePoint::RectIntersect)"<<eps[0]->rectIntersect<<");"
                // << endl;
                topology::Edge* e
                    = new topology::Edge(clusterCount, idealCircumference, eps);
...
)
{
    ...
    topology::TopologyConstraints
         t(dim, topologyNodes, topologyRoutes, clusterHierarchy, vs, cs);
    bool interrupted;
    int  loopBreaker = 100;
    do {
        interrupted = t.solve();
...
double ColaTopologyAddon::applyForcesAndConstraints(
    ...
    vpsc::Constraints&           cs,
    std::valarray<double>&       coords,
    cola::DesiredPositionsInDim& des,
    double                       oldStress
)
{
    ...
    topology::setNodeVariables(topologyNodes, vs);
    topology::TopologyConstraints
        t(dim, topologyNodes, topologyRoutes, layout->clusterHierarchy, vs, cs);
    bool            interrupted;
    int             loopBreaker = 100;
