...
using LayoutEdgeSegmentSeparations = std::set<LayoutEdgeSegmentSeparation>;
...
using LayoutEdgeSegmentList = std::list<LayoutEdgeSegment*>;
...
using LayoutObstacleVector = std::vector<LayoutObstacle>;
...
static bool insideLayoutObstacleBounds(
    const Point&          point,
...
using LayoutScanlineNodeSet = std::set<Node*, LayoutScanlineCmpNodePos>;
...
// from the scanline.
static void processLayoutConstraintEvent(
...
)
{
    ...
    if ((e->type == SegClose) || (e->type == SegOpen))
    {
        ...
        {
            double sepDist = beforeLayoutV->layoutObstacle->halfSizeInDim(dim);
        ...
        {
            double sepDist = afterLayoutV->layoutObstacle->halfSizeInDim(dim);
    ...
    if ((e->type == Close) || (e->type == Open))
    {
        ...
        if (ln->side == SIDE_LEFT)
        {
            ...
            else if (beforeLes)
            {
                ...
                double sepDist = ln->layoutObstacle->halfSizeInDim(dim);
                COLA_ASSERT(sepDist >= 0);
    ...
    if (e->type == SegClose)
    {
             ...
             ++it)
        {
            ...
            if (otherLes && (les->connRef == otherLes->connRef)
                && !(les->containsCheckpoint) && !(otherLes->containsCheckpoint)
                && (les->highPoint()[altDim] == otherLes->lowPoint()[altDim]))
            {
                ...
                newLess.distance = fabs(
                    les->highPoint()[les->dimension]
...
using EndpointAnchorList = std::list<EndpointAnchorInMoveDir>;

static void buildOrthogonalLayoutSegments(
    Router*                router,
    const size_t           dim,
         ...
         ++curr)
    {
        ...
        for (size_t i = 1; i < displayRoute.size(); ++i)
        {
            ...
            if (displayRoute.ps[i - 1][dim] == displayRoute.ps[i][dim])
            {
                ...
                size_t indexHigh = i;
                COLA_ASSERT(
                    displayRoute.ps[i - 1][altDim] != displayRoute.ps[i][altDim]
                );
                ...
                if ((i == 1) || ((i + 1) == displayRoute.size()))
                {
                    ...
                    bool last = ((i + 1) == displayRoute.size()) ? true : false;
                ...
                LayoutEdgeSegment* nss = new LayoutEdgeSegment(
                    *curr,
                    indexLow,
                    indexHigh,
                    isSBend,
                ...
                if (i == 1)
                {
                    ...
                    for (size_t k = 0; k < obstacleVector.size(); ++k)
                    {
                            ...
                            ))
                        {
                            ...
                            EndpointAnchorInMoveDir
                                terminal(dim, *curr, i - 1, k, offset);
...
static void setupOrthogonalLayoutConstraints(
    Router*                    router,
    const size_t               dim,
    LayoutEdgeSegmentList&     segmentList,
    LayoutObstacleVector&      obstacleVector,
...
)
{
    ...
    for (unsigned i = 0; i < ovn; i++)
    {
        ...
        bool     reverse = false;
        unsigned colaIndex
            = idMap.mappingForVariable(obstacle.shape()->id(), reverse);
        ...
        if (colaIndex < rs.size())
        {
            ...
            fprintf(
                stderr,
                "Shape %03d (cola: %03d) centre offset %g "
                "(from %g, %g)\n",
                index,
                colaIndex,
                offsetsVector[i],
    ...
    for (cola::CompoundConstraints::const_iterator c = ccs.begin();
         ...
         ++c)
    {
        (*c)->generateSeparationConstraints(
            (vpsc::Dim)dim,
            vs,
            valid,
            boundingBoxes
    ...
    while (!less.empty())
    {
        ...
        valid.push_back(
            new vpsc::Constraint(sepChoice.var1, sepChoice.var2, 0, true)
        );

#ifdef ORTHOG_TOPOLOGY_DEBUG
        fprintf(
            stderr,
            "Try removing sep in %c on connector %03d...",
            (dim == XDIM) ? 'X' : 'Y',
            sepChoice.connRef->id()
        );
#endif
                 ...
                 it != valid.end();)
            {
                        ...
                        < 0.01))
                {
                    ...
                    fprintf(
                        ...
                        "%s constraint: "
                        "(%d)%.12g + %g %s (%d)%.12g\n",
                        constraint->equality ? "Dropped redundant" : "Relaxed",
                        constraint->left->id,
                        constraint->left->finalPosition,
                        constraint->gap,
                        constraint->equality ? "==" : "<=",
                        constraint->right->id,
                        constraint->right->finalPosition
        ...
        for (size_t i = 0; i < valid.size(); ++i)
        {
            if (valid[i]->unsatisfiable)
            {
                ...
                fprintf(
                    stderr,
                    "Unsatisfiable: (%d)%.12g + %g %s (%d)%.12g\n",
                    valid[i]->left->id,
                    valid[i]->left->finalPosition,
                    valid[i]->gap,
                    valid[i]->equality ? "==" : "<=",
                    valid[i]->right->id,
        ...
        else
        {
            ...
            filename << "layout-" << dim << "-" << std::setfill('0')
                     << std::setw(3) << count;
    ...
    {
        cola::CompoundConstraint* cc = ccs[i];
        cc->updateShapeOffsetsForDifferentCentres(offsetsVector, reversed);
...
void AvoidTopologyAddon::improveOrthogonalTopology(Router* router)
{
    ...
    for (size_t dimension = 0; dimension < 2; ++dimension)
    {
        ...
        setupOrthogonalLayoutConstraints(
            router,
            dimension,
            segmentList,
            obstacleVector,
    ...
    if (fp)
    {
        ...
        for (size_t i = 0; i < m_rectangles.size(); ++i)
        {
            fprintf(
                ...
                m_rectangles[i]->getMinX(),
                m_rectangles[i]->getMaxX(),
                m_rectangles[i]->getMinY(),
                m_rectangles[i]->getMaxY()
            );
        ...
        else
        {
            fprintf(
                fp,
                "    RootCluster *cluster%llu = nullptr;\n\n",
        ...
        fprintf(
            fp,
            "    topology::AvoidTopologyAddon topologyAddon(rs, ccs, "
            "cluster%llu, idMap);\n",
            (unsigned long long)m_cluster_hierarchy
        );
