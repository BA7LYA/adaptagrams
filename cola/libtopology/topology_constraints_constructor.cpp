...
struct SegmentOpen;
using OpenSegments = list<SegmentOpen*>;

struct NodeOpen;
using OpenNodes = map<double, NodeOpen*>;
...
/*
 * There is a NodeEvent for the top and bottom (or left and right sides
 * depending on scan direction) of the rectangle associated with each node
 */
struct NodeEvent : Event
{
    ...
    void createStraightConstraints(
        ...
        const Node*   leftNeighbour,
        const Node*   rightNeighbour
    );
};
...
struct NodeOpen : NodeEvent
{
    ...
    void process(OpenNodes& openNodes, OpenSegments& openSegments)
    {
        ...
        if (!r.second)
        {
            ...
            printf(
                "scanpos %f, duplicate in open list at position: %f\n",
                pos,
...
struct NodeClose : NodeEvent
{
    ...
    {
        FILE_LOG(logDEBUG) << "NodeClose::createNonOverlapConstraint left="
                           << left << " right=" << right;
        // double overlap =
        // left->rect->overlapD(vpsc::conjugate(scanDim),right->rect);
        // if(overlap>1e-5) {
...
struct SegmentEvent : Event
{
    ...
    SegmentEvent(vpsc::Dim dim, bool open, EdgePoint* v, Segment* s)
        : Event(open, v->pos(vpsc::conjugate(dim)))
        , s(s)
    {
    }
};
...
/*
 * at a segment closing we remove the segment from the list of openings and
 * cleanup
 */
struct SegmentClose : SegmentEvent
{
    ...
    SegmentClose(vpsc::Dim dim, Segment* s, SegmentOpen* so)
        : SegmentEvent(dim, false, s->getMax(dim), s)
        , opening(so)
    {
...
{
    FILE_LOG(logDEBUG) << "NodeEvent::createStraightConstraints():node->id="
                       << node->id << " pos=" << pos;
    const double leftLimit
        = leftNeighbour ? leftNeighbour->rect->getCentreD(scanDim) : -DBL_MAX,
        rightLimit
        = rightNeighbour ? rightNeighbour->rect->getCentreD(scanDim) : DBL_MAX;
    for (OpenSegments::iterator j = openSegments.begin();
         j != openSegments.end();
         ++j)
    {
        Segment* s = (*j)->s;
        ...
        if ((p < leftLimit
             && pos > leftNeighbour->rect->getMinD(vpsc::conjugate(scanDim))
             && pos < leftNeighbour->rect->getMaxD(vpsc::conjugate(scanDim)))
            || (p > rightLimit
                && pos > rightNeighbour->rect->getMinD(vpsc::conjugate(scanDim))
...
bool Segment::createStraightConstraint(
    vpsc::Dim scanDim,
    Node*     node,
    double    pos
)
{
    ...
    const double top = max(
                     ...
                     start->pos(vpsc::conjugate(scanDim))
                 ),
                 bottom = min(
                     end->pos(vpsc::conjugate(scanDim)),
                     start->pos(vpsc::conjugate(scanDim))
    ...
    FILE_LOG(logDEBUG) << "Segment::createStraightConstraint, node->id="
                       << node->id << ", edge->id=" << edge->id
                       << " pos=" << pos;
    if (top == bottom)
    {
        FILE_LOG(logDEBUG1
        ) << "  Not creating because segment is orthogonal to scan direction!";
        return false;
    }
    ...
    FILE_LOG(logDEBUG1) << "Segment: from {" << start->pos(scanDim) << ","
                        << start->pos(vpsc::conjugate(scanDim)) << "},{"
                        << end->pos(scanDim) << ","
                        << end->pos(vpsc::conjugate(scanDim)) << "}";
    ...
    double p;
    bool   nodeLeft
        = r->getCentreD(scanDim) < forwardIntersection(scanDim, pos, p);
    // set ri (the vertex of the node rectangle that is to be
    // kept to the left of the segment
    EdgePoint::RectIntersect ri;
    if (scanDim == vpsc::HORIZONTAL)
    {
        ri = pos < r->getCentreY() ? (nodeLeft ? EdgePoint::BR : EdgePoint::BL)
                                   : (nodeLeft ? EdgePoint::TR : EdgePoint::TL);
    }
    else
    {
        ri = pos < r->getCentreX() ? (nodeLeft ? EdgePoint::TL : EdgePoint::BL)
                                   : (nodeLeft ? EdgePoint::TR : EdgePoint::BR);
    }
    if (node->id == start->node->id && ri == start->rectIntersect)
    {
        FILE_LOG(logDEBUG1
        ) << "Not creating StraightConstraint because bend point is already a "
             "real bend associated with the start EdgePoint of this segment!";
        return false;
    }
    if (node->id == end->node->id && ri == end->rectIntersect)
    {
        FILE_LOG(logDEBUG1)
            << "Not creating StraightConstraint because bend point is already "
               "a real bend associated with the end EdgePoint of this segment!";
        return false;
    }
    ...
    , pos(scanPos)
{
    FILE_LOG(logDEBUG) << "StraightConstraint ctor: pos=" << pos
                       << " edge id=" << s->edge->id << " node id=" << node->id;
    EdgePoint *u = s->start, *v = s->end;
    FILE_LOG(logDEBUG1) << "s->start: id=" << u->node->id
                        << ", ri=" << u->rectIntersect << ", x=" << u->posX()
                        << ", y=" << u->posY();
    FILE_LOG(logDEBUG1) << "node:     id=" << node->id << ", ri=" << ri
                        << ", scanpos=" << scanPos;
    FILE_LOG(logDEBUG1) << "s->end:   id=" << v->node->id
                        << ", ri=" << v->rectIntersect << ", x=" << v->posX()
                        << ", y=" << v->posY();

    double g = u->offset(scanDim)
             + segmentPos * (v->offset(scanDim) - u->offset(scanDim));
    if (nodeLeft)
    {
...
BendConstraint::BendConstraint(EdgePoint* v, vpsc::Dim dim)
    ...
    , bendPoint(v)
{
    FILE_LOG(logDEBUG) << "BendConstraint ctor, pos="
                       << v->pos(vpsc::conjugate(dim));
    ...
    {
        if (v->rectIntersect == EdgePoint::TR
            || v->rectIntersect == EdgePoint::BR)
        {
            leftOf = true;
        }
    }
    ...
    {
        if (v->rectIntersect == EdgePoint::TL
            || v->rectIntersect == EdgePoint::TR)
        {
            leftOf = true;
        }
    }
    ...
    FILE_LOG(logDEBUG1) << "w: id=" << w->node->id
                        << ", ri=" << w->rectIntersect << ", x=" << w->posX()
                        << ", y=" << w->posY();
    // bend constraint will be more accurate if the reference segment is the
    // one most orthogonal to scan line.
    double p;
    if (v->inSegment->length(vpsc::conjugate(dim))
        > v->outSegment->length(vpsc::conjugate(dim)))
    {
        v->inSegment
            ->forwardIntersection(scanDim, w->pos(vpsc::conjugate(dim)), p);
        double g = u->offset(scanDim)
                 + p * (v->offset(scanDim) - u->offset(scanDim))
                 - w->offset(scanDim);
    ...
    {
        v->outSegment
            ->reverseIntersection(scanDim, u->pos(vpsc::conjugate(dim)), p);
        double g = w->offset(scanDim)
                 + p * (v->offset(scanDim) - w->offset(scanDim))
                 - u->offset(scanDim);
...
inline bool validTurn(EdgePoint* u, EdgePoint* v, EdgePoint* w)
{
    ...
    double           cpuvr
        = crossProduct(u->posX(), u->posY(), v->posX(), v->posY(), rx, ry);
    double cpvwr
        = crossProduct(v->posX(), v->posY(), w->posX(), w->posY(), rx, ry);
    ...
    void operator()(EdgePoint* p)
    {
        if (p->inSegment && p->outSegment)
        {
            ...
            if (inSegLen > 0 && outSegLen > 0
                && o->pos(vpsc::conjugate(scanDim))
                       == p->pos(vpsc::conjugate(scanDim))
                && p->pos(vpsc::conjugate(scanDim))
                       == q->pos(vpsc::conjugate(scanDim)))
            {
...
static void recCreateTopologyClusterNodes(
    cola::Cluster*  cluster,
    const vpsc::Dim dim,
    Nodes&          clusterNodes
)
{
    ...
    if (rectCluster)
    {
        clusterNodes.push_back(new Node(
            rectCluster->clusterVarId,
            rectCluster->getMinEdgeRect(dim),
            (dim == 0) ? rectCluster->vXMin : rectCluster->vYMin
        ));
        clusterNodes.push_back(new Node(
            rectCluster->clusterVarId + 1,
    ...
    , dim(axisDim)
{
    ...
    FILE_LOG(logDEBUG) << "TopologyConstraints::TopologyConstraints():dim="
                       << axisDim;
    ...
    // the event queue
    for (Nodes::const_iterator i = allNodes.begin(), e = allNodes.end(); i != e;
         ++i)
    {
        Node*      v     = *i;
    ...
    {
        (*i)->forEach(
            CreateBendConstraints(dim),
            CreateSegmentEvents(events, dim),
            true
        );
    }
    // process events in top to bottom order
    sort(events.begin(), events.end(), CompareEvents());
    for (vector<Event*>::iterator curr = events.begin(); curr != events.end();
         ++curr)
    {
...
TopologyConstraints::~TopologyConstraints()
{
    ...
    {
        (*i)->forEach(
            mem_fn(&EdgePoint::deleteBendConstraint),
            mem_fn(&Segment::deleteStraightConstraints),
            true
        );
    }
}
