...
/*
 * Method body and support routines for
 * topology::TopologyConstraints::computeForces.
 */
...
static double gRule1(vpsc::Dim dim, const EdgePoint* a, const EdgePoint* b)
{
    ...
    double lab = dim == vpsc::HORIZONTAL ? len(a, b, dxab, dyab, dxab2, dyab2)
                                         : len(a, b, dyab, dxab, dyab2, dxab2);
    COLA_ASSERT(lab != 0);
    return dxab / lab;
}

static double gRule2(
    vpsc::Dim        dim,
    const EdgePoint* a,
    const EdgePoint* b,
    const EdgePoint* c
)
{
    ...
    double lbc = dim == vpsc::HORIZONTAL ? len(b, c, dxbc, dybc, dxbc2, dybc2)
                                         : len(b, c, dybc, dxbc, dybc2, dxbc2);
    COLA_ASSERT(lbc != 0);
    return dxab / lab - dxbc / lbc;
}

static double hRuleD1(
    vpsc::Dim        dim,
    const EdgePoint* u,
    const EdgePoint* v,
    const double     dl
)
{
    ...
    double l = dim == vpsc::HORIZONTAL ? len(u, v, dx, dy, dx2, dy2)
                                       : len(u, v, dy, dx, dy2, dx2);
    COLA_ASSERT(l != 0);
    return dl * (dx2 / (l * l * l) - 1 / l) + dx2 / (l * l);
}

static double hRuleD2(
    vpsc::Dim        dim,
    const EdgePoint* u,
    const EdgePoint* v,
    const EdgePoint* w,
...
)
{
    ...
    double luv = dim == vpsc::HORIZONTAL ? len(u, v, dxuv, dyuv, dxuv2, dyuv2)
                                         : len(u, v, dyuv, dxuv, dyuv2, dxuv2);
    COLA_ASSERT(luv != 0);
    double dxvw, dyvw, dxvw2, dyvw2;
    double lvw = dim == vpsc::HORIZONTAL ? len(v, w, dxvw, dyvw, dxvw2, dyvw2)
                                         : len(v, w, dyvw, dxvw, dyvw2, dxvw2);
    COLA_ASSERT(lvw != 0);
    double p1 = dl
              * (dxuv2 / (luv * luv * luv) - 1 / luv + dxvw2 / (lvw * lvw * lvw)
                 - 1 / lvw);
    double p2 = (dxuv / luv - dxvw / lvw);
    return p1 + p2 * p2;
}

static double hRule2(
    vpsc::Dim        dim,
    const EdgePoint* u,
    const EdgePoint* v,
    const EdgePoint* w,
...
)
{
    ...
    double lvw = dim == vpsc::HORIZONTAL ? len(v, w, dxvw, dyvw, dxvw2, dyvw2)
                                         : len(v, w, dyvw, dxvw, dyvw2, dxvw2);
    COLA_ASSERT(lvw != 0);
    return -dl * dxuv2 / (luv * luv * luv) + dl / luv - dxuv2 / (luv * luv)
         + dxuv * dxvw / (luv * lvw);
}

static double hRule3(
    vpsc::Dim        dim,
    const EdgePoint* u,
    const EdgePoint* v,
    const EdgePoint* w,
...
)
{
    ...
    double lvw = dim == vpsc::HORIZONTAL ? len(v, w, dxvw, dyvw, dxvw2, dyvw2)
                                         : len(v, w, dyvw, dxvw, dyvw2, dxvw2);
    COLA_ASSERT(lvw != 0);
    return -dl * dxvw2 / (lvw * lvw * lvw) - dxvw2 / (lvw * lvw) + dl / lvw
         + dxuv * dxvw / (luv * lvw);
}

static double hRule4(
    vpsc::Dim        dim,
    const EdgePoint* a,
    const EdgePoint* b,
    const EdgePoint* c,
...
)
{
    ...
    double lcd = dim == vpsc::HORIZONTAL ? len(c, d, dxcd, dycd, dxcd2, dycd2)
                                         : len(c, d, dycd, dxcd, dycd2, dxcd2);
    COLA_ASSERT(lcd != 0);
    return -dxab * dxcd / (lab * lcd);
}

static double hRule56(
    vpsc::Dim        dim,
    const EdgePoint* u,
    const EdgePoint* v,
    const EdgePoint* a,
...
/*
 * Compute the forces associated with each EdgePoint (bend or end point) along
 * each each on the nodes/rectangles in the graph.
 */
void TopologyConstraints::computeForces(
