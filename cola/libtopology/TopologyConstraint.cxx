...
#include "libtopology/TopologyConstraint.hxx"
...
{
    const double u1 = u->initialPos(scanDim), u2 = u->finalPos(),
                 v1 = v->initialPos(scanDim), v2 = v->finalPos(),
                 w1 = w->initialPos(scanDim), w2 = w->finalPos(),
                 fSlack = slackAtFinal();
    if (fSlack >= 0)
    {
        FILE_LOG(logDEBUG1)
            << "TriConstraint::maxSafeAlpha(): slackAtFinal=" << slackAtFinal();
        return 1;
    }
    ...
    FILE_LOG(logDEBUG1) << "TriConstraint::maxSafeAlpha(): num=" << numerator
                        << " den=" << denominator;
    FILE_LOG(logDEBUG1) << "  u1=" << u1 << " u2=" << u2 << " v1=" << v1
                        << " v2=" << v2 << " w1=" << w1 << " w2=" << w2;
    if (denominator == 0)
    {
        return 1;
...
double TriConstraint::slack(const double ux, const double vx, const double wx)
    const
{
    const double lhs = wx;
    const double rhs = ux + p * (vx - ux) + g;
    FILE_LOG(logDEBUG1) << "  TriConstraint::slack(" << ux << "," << vx << ","
                        << wx << "):leftOf=" << leftOf << ",lhs=" << lhs
                        << ",rhs=" << rhs;
...
{
    return slack(u->finalPos(), v->finalPos(), w->finalPos());
}

double TriConstraint::slackAtInitial() const
{
    return slack(
        u->initialPos(scanDim),
        v->initialPos(scanDim),
...
{
    double ux = c.u->finalPos(), vx = c.v->finalPos(), wx = c.w->finalPos();
    os << "TriConstraint@" << &c << ": u=" << ux << " v=" << vx << " w=" << wx
       << " p=" << c.p << " g=" << c.g << " left=" << c.leftOf;
    return os;
}
...
void BendConstraint::satisfy()
{
    ...
    FILE_LOG(logDEBUG) << "BendConstraint::satisfy()... edge id=" << getEdgeID()
                       << " node id=" << bendPoint->node->id;
...
struct transferStraightConstraintChoose
{
     ...
     */
    transferStraightConstraintChoose(
        Segment*            target1,
        Segment*            target2,
        ...
        : ignore(ignore)
    {
        ...
        double    min1 = min(
            target1->start->pos(vpsc::conjugate(dim)),
            target1->end->pos(vpsc::conjugate(dim))
        );
        double max1 = max(
            target1->start->pos(vpsc::conjugate(dim)),
            target1->end->pos(vpsc::conjugate(dim))
        ...
        double min2 = min(
            target2->start->pos(vpsc::conjugate(dim)),
            target2->end->pos(vpsc::conjugate(dim))
        );
        double max2 = max(
            target2->start->pos(vpsc::conjugate(dim)),
            target2->end->pos(vpsc::conjugate(dim))
...
bool sameCorner(const EdgePoint* a, const EdgePoint* b)
{
    ...
    COLA_UNUSED(b);
    COLA_ASSERT(
        !(a->node->id == b->node->id && a->rectIntersect == b->rectIntersect)
    );
    return false;
}
...
void StraightConstraint::satisfy()
{
    ...
    Edge*      e     = segment->edge;
    EdgePoint *start = segment->start, *end = segment->end,
              *bend = new EdgePoint(node, ri);
    FILE_LOG(logDEBUG1) << "  u=(" << start->node->id << ":"
                        << start->rectIntersect << "), v=(" << node->id << ":"
                        << ri << "), w=(" << end->node->id << ":"
                        << end->rectIntersect << ")";
...
string StraightConstraint::toString() const
{
    stringstream s;
    s << "StraightConstraint: node id=" << node->id << ", segment=("
      << segment->start->node->id << ":" << segment->start->rectIntersect << ","
      << segment->end->node->id << ":" << segment->end->rectIntersect
      << ") pos=" << pos;
    return s.str();
}
...
struct buildRoute
{
    buildRoute(straightener::Route* r, unsigned& n)
        : r(r)
        , n(n)
    {
    }
...
void TopologyConstraints::constraints(std::vector<TopologyConstraint*>& ts
) const
{
    for_each(
        edges.begin(),
        edges.end(),
        bind(&Edge::getTopologyConstraints, std::placeholders::_1, &ts)
    );
}
...
struct PrintEdgePoint
{
    ...
    {
        printf(
            "t.addToPath(%d,(topology::EdgePoint::RectIntersect)%d);\n",
            p->node->id,
            p->rectIntersect
        );
    }
...
void TopologyConstraints::printInstance(valarray<double>& g) const
{
...
bool TopologyConstraints::assertFeasible() const
{
...
bool TopologyConstraints::solve()
{
    ...
    for (vector<TopologyConstraint*>::iterator i = ts.begin(); i != ts.end();
         ++i)
    {
