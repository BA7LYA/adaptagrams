...
#include "libproject/Project.hxx"
...
#define ASSERT_NONE_VIOLATED(p)                                                \
    {                                                                          \
        for (Constraints::const_iterator i = (p)->cs.begin();                  \
             i != (p)->cs.end();                                               \
             i++)                                                              \
        {                                                                      \
            Constraint* c = *i;                                                \
            double XIl = c->l->block->XI, XIr = c->r->block->XI, bl = c->l->b, \
                   br    = c->r->b;                                            \
...
double Variable::relativeInitialPos() const
{
    return block->XI + b;
}

double Variable::relativeDesiredPos() const
{
    return block->X + b;
}
...
/**
 * Used by computeLagrangians to recursively compute lagrangrians over the tree
 * of active constraints, from the partial derivatives of each variable.
 * @param v either the left or right side of last, used as the starting point of
 * recursion
 * @param last don't backtrack over this constraint.
 */
double compute_dfdv(const Variable* v, const Constraint* last)
{
...
Project::Project(const vector<Variable*>& vs, const vector<Constraint*>& cs)
    : vs(vs)
    , cs(cs)
 ...
 */
void Project::initBlocksAndConstraints()
{
    ...
    for_each(
        cs.begin(),
        cs.end(),
        bind2nd(mem_fun(&Constraint::setActive), false)
    );
}

/**
 * @return the maximum move we can make along the line from initial to desired
 * positions without violating this constraint
 */
double Constraint::maxSafeAlpha() const
{
    ...
    else
    {
        ...
        double Al = l->relativeInitialPos(), Ar = r->relativeInitialPos(),
               Bl = l->block->toDesired(), Br = r->block->toDesired();
        double n = g + Al - Ar, d = Br - Bl;
...
/**
 * Functor used for finding the largest move (alpha) we can make along the line
 * from current positions to desired positions without violating a constraint.
 */
struct MaxSafeMove : unary_function<Constraint*, void>
{
    MaxSafeMove(Constraint*& c, double& alpha)
        : c(c)
        , alpha(alpha)
    {
    }
    ...
    /**
     * Compute the distance along the line from current to desired positions we
     * would need to move to make a given constraint tight.  If that distance is
     * smaller than the existing alpha then store it in alpha and note the
     * constraint in c.
    ...
    double&
        alpha;  ///< the distance required to move c in order to make it tight
};
 ...
 * If the function pointer externalAlphaCheck is set, then the function it
 * points to is called giving the application the chance to interrupt the solver
 * early (for example, to satisfy some other type of constraint and recompute
 * the desired positions).
 * @param c will be set to the constraint which becomes active at alpha
 * @return the largest move (alpha) that we can make without violating a
 * constraint
 */
double Project::findSafeMove(Constraint*& c)
{
    c            = nullptr;
    double alpha = 1.0;
...
{
    os << "(" << v->getPosition() << "," << v->getDesiredPosition() << ","
       << v->getWeight() << ")";
    return os;
}
 ...
 * The desired position of the merged block is recomputed and a "virtual"
 * initial position for the new block is computed by projection along the line
 * from the new desired position and the point XI+alpha*(X-XI), where XI and X
 * are the initial and desired positions of the old left block.
 ...
 * each block
 */
void Project::makeActive(Constraint* c, double alpha)
{
    Block* L          = c->l->block;
    Block* R          = c->r->block;
    ...
    L->XI = (L->XI - alpha * (L->XI - prevOptPos + L->X)) / (1.0 - alpha);
...
bool cmpLagrangians(Constraint* a, Constraint* b)
{
    return a->lm < b->lm;
}
 ...
 * closer to their desired positions.  Returns true if no further splits are
 * required and therefore an optimal solution has been found.
 */
bool Project::splitBlocks()
{
    bool optimal = true;
    for (Blocks::iterator i = blocks.begin(); i != blocks.end(); ++i)
    {
        ...
        Constraint* sc = *min_element(b->C.begin(), b->C.end(), cmpLagrangians);
...
/**
 * Populate a new block that is created as the result of a splitting an existing
 * block by traversing its tree of active constraints.
 * @param v the start point of the traversal, assumes v->block still points to
 * the old block
 * @param last don't backtrack over this constraint
 */
void Block::populateSplitBlock(Variable* v, const Constraint* last)
{
 ...
 */
Blocks::iterator Project::makeInactive(Constraint* c)
{
