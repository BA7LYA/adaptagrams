...
/**
 * project namespace delineates classes belonging to the libproject solver
 * library.
 */
namespace project {
...
public:
    ...
    Variable(const Initial& i, const Desired& d, const Weight& w = Weight(1.0))
        : id(idCtr++)
        , x(i.pos)
        , d(d.pos)
        , w(w.w)
     ...
     */
    void setPosition(const Desired& des)
    {
        d = des.pos;
    }
    ...
    void setPosition(const Initial& p)
    {
        x = p.pos;
    }
...
private:
    ...
    friend double compute_dfdv(const Variable*, const Constraint*);
};
...
private:
    ...
    friend class Block;
    friend double compute_dfdv(const Variable* v, const Constraint* last);
...
public:
    ...
    /**
     * Compute the optimal position for this block based on the ideal positions
     * of its constituent variables.
     */
    double optimalPosition() const;

    /**
     * compute the lagrangian multipliers of all the active constraints in this
     * block.
     */
    void computeLagrangians();

    /// vector to desired position
    double toDesired() const
    {
        return X - XI;
    }
...
private:
    /**
     * Populate a new block that is created as the result of a splitting an
     * existing block by traversing its tree of active constraints.
     * @param v the start point of the traversal, assumes v->block still points
     * to the old block
     * @param last don't backtrack over this constraint
     */
    void populateSplitBlock(Variable* v, const Constraint* last);
};
...
private:
     ...
     * points to is called after an alpha is found for the internal constraints.
     * This gives the application the chance to interrupt the solver early (for
     * example, to satisfy some other type of constraint and recompute the
     * desired positions).
    ...
    /**
     * Repeatedly search along the line from current to desired positions for
     * the first constraint that would be violated if we moved any further, and
     * make that constraint active.  We finish when all blocks can be moved to
     * their desired positions without violating any further constraints.
     */
    void makeOptimal();

    /**
     * Make the specified constraint active by setting to equality and merging
     * the two blocks that it spans into one new block (actually we merge the
     * right hand side into the left). The desired position of the merged block
     * is recomputed and a "virtual" initial position for the new block is
     * computed by projection along the line from the new desired position and
     * the point XI+alpha*(X-XI), where XI and X are the initial and desired
    ...
    /**
     * Make a given active constraint inactive, therefore cutting the tree of
     * active constraints in the block to which it belongs, and creating two new
     * blocks.
     */
    Blocks::iterator makeInactive(Constraint* c);

    /**
     * Check each block to see if splitting it allows the two new blocks to be
     * moved closer to their desired positions.  Returns true if no further
     * splits are required and therefore an optimal solution has been found.
     */
    bool splitBlocks();
...
/**
 * Critical failure: either something went wrong with the solver or (more
 * likely) there was infeasible input.
 */
struct CriticalFailure
{
    ...
    void print()
    {
        fprintf(
            stderr,
            "ERROR: Critical sanity check failed in libproject!\n"
            "  expression: %s\n  at line %d of %s\n"
            "  in: %s\n",
            expr,
            line,
